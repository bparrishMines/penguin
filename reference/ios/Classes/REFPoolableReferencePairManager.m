...
+ (REFPoolableReferencePairManager *_Nullable)
    managerFromClass:(NSSet<REFReferencePairManagerPool *> *_Nonnull)pools
...
+ (REFPoolableReferencePairManager *_Nullable)
    managerFromPoolID:(NSSet<REFReferencePairManagerPool *> *_Nonnull)pools
...
+ (id<REFLocalReference> _Nullable)
    localRefFromRemoteRef:(NSSet<REFReferencePairManagerPool *> *_Nonnull)pools
          remoteReference:(REFRemoteReference *_Nonnull)remoteReference {
  for (REFReferencePairManagerPool *pool in pools) {
    NSEnumerator<REFPoolableReferencePairManager *> *enumerator = pool.managers.objectEnumerator;
    REFPoolableReferencePairManager *manager;

    while ((manager = [enumerator nextObject])) {
      id<REFLocalReference> localReference = [manager getPairedLocalReference:remoteReference];
...
- (id)convertReferencesForRemoteManager:(REFReferencePairManager *)manager obj:(id)obj {
  ...
  REFPoolableReferencePairManager *correctManager =
      isCorrectManager
          ? poolableManager
  ...
  return [[REFUnpairedReference alloc]
        initWithClassID:[correctManager getClassID:localReference.referenceClass]
      creationArguments:
          [correctManager.converter
...
- (id)convertReferencesForLocalManager:(REFReferencePairManager *)manager obj:(id)obj {
  if (![obj isKindOfClass:[REFRemoteReference class]] &&
      ![obj isKindOfClass:[REFUnpairedReference class]]) {
  ...
  if (objIsRemoteReference && [manager getPairedLocalReference:obj]) {
    ...
    return [REFPoolableReferenceConverter localRefFromRemoteRef:poolableManager.pools
                                                remoteReference:obj];
  }

  REFUnpairedReference *unpairedReference = obj;
  REFPoolableReferencePairManager *correctManager =
      [poolableManager.poolID isEqualToString:unpairedReference.managerPoolID]
          ? poolableManager
          : [REFPoolableReferenceConverter managerFromPoolID:poolableManager.pools
                                                      poolID:unpairedReference.managerPoolID];

  return [correctManager.localHandler
              create:correctManager
